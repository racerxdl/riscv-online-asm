...
CodeMirror.defineMode("gas", function(_config, parserConfig) {
  ...
  function riscv(_parserConfig) {
    ...
    for (let i = 0; i < 32; i++) {
      registers[`x${i}`] = "variable-2";
    }

    for (let i = 0; i < 32; i++) {
      registers[`f${i}`] = "variable-2";
    }
    ...
    for (let i = 0; i < 8; i++) {
      registers[`a${i}`] = registers[`x${i+10}`]; 
    }
    for (let i = 2; i < 12; i++) {
      registers[`s${i}`] = registers[`x${i+18-2}`]; 
    }
    for (let i = 3; i < 7; i++) {
      registers[`t${i}`] = registers[`x${i+28-3}`]; 
    }
